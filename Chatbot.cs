...
namespace CybersecurityChatbotWPF
{
    public class Chatbot
    {
        ...
        public void ProcessUserInput(string question)
        {
            ...
            {
                string favoriteTopic = question.ToLower().Trim();
                if (!string.IsNullOrWhiteSpace(favoriteTopic))
                {
                    userMemory["favoriteTopic"] = favoriteTopic;
                    OnBotResponseReady($"Great! I'll remember that you're interested in {favoriteTopic}.");
            ...
            else if (question.Contains("complete task") || question.Contains("mark task complete"))
            {
        ...
        public void SetReminderDate(string reminderInput)
        {
            ...
            userMemory.Remove("currentTaskTitle"); // Clear current task context
            ...
            LogActivity($"Added task: \"{title}\" with reminder: {(reminderDate.HasValue ? reminderDate.Value.ToShortDateString() : "None")}.");
        }
        ...
        public void MarkTaskComplete(string input)
        {
            ...
            if (int.TryParse(input, out int taskNumber) && taskNumber > 0 && taskNumber <= tasks.Count)
            {
                ...
                LogActivity($"Marked task \"{tasks[taskNumber - 1].Title}\" as complete.");
            }
            else
            {
        ...
        public void DeleteTask(string input)
        {
            ...
            if (int.TryParse(input, out int taskNumber) && taskNumber > 0 && taskNumber <= tasks.Count)
            {
                ...
                LogActivity($"Deleted task: \"{deletedTaskTitle}\".");
            }
            else
            {
                OnBotResponseReady("Invalid task number.");
        ...
        private void ProcessQuizAnswer(string userAnswer)
        {
            ...
            if (userAnswer.ToLower() == correctAnswer ||
                (userAnswer.Length == 1 && (userAnswer.ToUpper() == "A" || userAnswer.ToUpper() == "B" || userAnswer.ToUpper() == "C" || userAnswer.ToUpper() == "D") && userAnswer.ToUpper() == currentQuestion.CorrectAnswer.ToUpper()))
            {
                OnBotResponseReady("Correct! " + currentQuestion.Explanation);
                correctAnswers++;
                LogActivity($"Answered quiz question {currentQuizQuestionIndex + 1} correctly.");
            }
            else
            {
                OnBotResponseReady("Incorrect. " + currentQuestion.Explanation);
        ...
        private void EndQuiz(bool prematurely = false)
        {
            ...
            quizResult.AppendLine($"Quiz finished! You answered {correctAnswers} out of {currentQuizQuestionIndex} questions correctly.");
            double percentage = (double)correctAnswers / currentQuizQuestionIndex * 100;
